...
|---|---|---|
| **Extraktionshelfer** | jeden n-ten Buchstaben, Indexliste („3,1,4"), aus jedem Wort, Gitter zeilen-/spalten-/diagonal lesen, ein Zeichen zählen, Länge nach Buchstaben, Ziffern, Zeichen, Sonderzeichen oder Wörtern, Zeichen ersetzen (Liste gegen Liste, in einem Durchgang; ohne Gegenstück gelöscht) – der Schlussschritt fast jedes Rätsels | S |
...
- **Upload nur nach grünen Tests** und nur vom Live-Branch (`LIVE_BRANCH` im Workflow, `main`),
  sonst bleibt die alte Version stehen. Soll vorübergehend ein anderer Branch live gehen,
  wird `LIVE_BRANCH` auf beiden Branches umgestellt – jeder Push läuft mit der
  Workflow-Datei seines eigenen Branches.
